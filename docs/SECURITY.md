...
- 手机审批 broker 只处理当前活动任务的命令执行、文件修改和非秘密提问，并使用绑定 task 的一次性随机令牌；秘密问题和未知 server request 始终拒绝
- `approvalPolicy: onRequest` 只应在真实手机审批链路现场验证后启用；无法及时处理审批时保留 `never` 可继续作为保守模式
- 飞书附件先按声明大小和实际大小双重限制，再进入任务专属临时目录；文件名不会作为本机相对或绝对路径使用，任务终态会清理临时目录
