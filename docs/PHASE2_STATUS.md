...
### ClawBridge 2.2.2：历史图片兼容与额度周期可读性

- 恢复 Codex 历史到飞书话题时，本机路径和其他 Markdown 图片不再作为飞书 `image_key` 提交，而是显示为“图片未同步”的文字占位；其余文字历史继续保留。
- 以整天计的额度窗口用“7 天”等格式显示，不再显示不直观的“168 小时”。

### ClawBridge 2.2.1：紧凑模型切换

- 对话话题底部的模型工具栏压缩为无标题的单行卡，只显示当前模型、推理强度和“切换”
- 模型选择卡移除模型介绍、对话说明和“返回项目”，每个模型只占一行
- 选择模型或推理强度后，原模型卡在原位收起为单行模型条，不会跳回或重新显示项目控制台
- 模型列表提供“收起”，同样在原卡片位置恢复单行模型条

### ClawBridge 2.2.0：话题底部模型入口与额度卡

- 项目群控制卡不再放置项目级“模型设置”；模型与推理强度继续按 Codex 对话保存
- 继续已有对话、以及每项话题任务完成后，会在该话题底部发送轻量会话工具栏；只保留“切换模型”，不提供“查看状态”
- 在已绑定对话的话题中单独发送“模型”或“模型设置”，可随时重新打开同一模型卡，不会把该文本送入 Codex
- 新建待绑定话题的首条任务仍使用默认模型；真实 Codex 对话创建并绑定后再显示工具栏
- 单聊控制台新增“剩余额度”，通过 `account/rateLimits/read` 读取主/次窗口、月度余量与重置时间，并提供原位刷新
- 额度功能不读取或显示飞书凭据、Codex 登录令牌或其他秘密字段
...
- 单聊控制台只负责选择项目、进入项目群、刷新和交还 Desktop；对话、项目任务和模型操作集中在项目群控制卡
- “新对话”不要求输入名称，只先创建待绑定话题；第一条话题任务启动后才创建真实 Codex 对话并完成绑定
...
- 每个项目最多绑定一个由机器人创建的私有飞书群；2.0.5 起群主聊天流使用普通消息模式承载项目控制卡，各 Codex 对话仍使用独立话题隔离
...
- 项目群控制卡提供“项目任务”入口，从 SQLite 任务快照读取并支持每页 10 条分页；文本命令仍可查看跨项目任务
...
- `thread/start` 创建的空线程在产生首个 rollout 前不能调用 `thread/resume`。客户端现在跟踪由当前 App Server 连接显式创建且尚未执行任务的线程：首条任务直接执行 `turn/start`，后续任务才执行 `thread/resume`。
- 线程被归档、退订或 App Server 停止时会清理这项临时状态，避免把历史线程误判为新空线程。
- 2026-08-13 验证：新增协议契约测试确认首条任务不发送 resume、第二条任务发送一次 resume；全量 Vitest 为 18 个测试文件、198 项测试全部通过，类型、格式和构建检查通过。

### v2.0.4 项目群生命周期补丁

- 再次打开已有项目群时不再盲信 SQLite 中保存的 `chat_id`：Bridge 会读取群状态并分页核对唯一授权用户是否仍在群内。
- 用户主动退出但原群仍存在时，Bridge 使用群成员接口将其重新邀请回原群，原有 Codex 对话、飞书话题路由和历史均保留。
- 群已解散或群 ID 失效时，Bridge 使用新的幂等键重建项目群，原 Codex 线程索引保持不变；旧群话题路由会在同一数据库事务中替换，并按当前选择自动建立新话题。
- 该能力需要 `im:chat:readonly`、`im:chat.members:read`、`im:chat.members:write_only` 和 `im:chat:operate_as_owner`；修改权限后需发布新飞书应用版本。
- 2026-08-13 验证：`npm run check`、`npm run build`、`npm run format:check` 通过；全量 Vitest 为 18 个测试文件、203 项测试全部通过。真实退群/重入和解散/重建仍需在发布权限后的飞书客户端现场验收。

### v2.0.5 群内项目控制台

- 新项目群改为普通消息模式，群主聊天流显示项目控制卡；每个 Codex 对话仍通过根消息加 `reply_in_thread` 建立独立话题，不把不同对话混入同一聊天流。
- 已存在的 thread-style 项目群在下一次从单聊打开时自动更新为普通消息模式，已有 Codex 历史和话题绑定不删除。
- 群内可查看/新建/选择当前项目的对话、查看项目任务及设置当前对话模型；列表与详情页使用“返回项目”，不会暴露全局项目选择器。
- 群卡动作同时校验已发送卡片证明、唯一授权用户、群 owner 以及项目 ID；跨项目卡片、全局项目管理和交还 Desktop 操作继续只允许在机器人单聊执行。
- 该模式更新需要应用具备群信息更新/以应用身份管理群的权限。代码与协议回归已覆盖旧群迁移、群内新对话创建和独立话题建立；真实飞书客户端 E2E 仍待发布应用版本后现场验收。
- 2026-08-14 验证：`npm run check`、`npm run build`、`npm run format:check`、`git diff --check` 全部通过；全量 Vitest 为 18 个测试文件、208 项测试全部通过。

### v2.1.0 话题优先对话流程

- 单聊控制台移除选择/新建对话、全局/项目任务和模型按钮，只保留项目选择、项目群入口、刷新与交还 Desktop。
- 已准备好的项目群入口改为纯 AppLink，避免卡片 callback 抢占跳转；退出丢弃后重新选择项目会先重建群，随后一次点击即可进入。
- 项目群“新建对话”不再调用 `thread/start` 创建空线程，而是写入数据库 v11 的待绑定话题；首条话题任务调用新线程 turn，`onStarted` 后原子升级为正式线程路由。

### v2.1.1 项目群重建入口修复

- 卡片通过 `im.v1.message.patch` 原位更新成功后，同时更新 deliveries 中的 fallback 文本、卡片 payload 和时间索引。
- 从“选择项目”改写而来的单聊控制台现在能被退出/丢弃流程准确找到并清除失效群链接。
- 兼容 2.1.0 遗留索引：解散流程直接更新同一单聊最近的已发送 P2P 卡片，避免更早的同名控制卡抢占定位，也避免用户反复刷新和重新选择项目。
- 对话列表只保留“继续对话”；旧 `thread.show` 卡片动作兼容重定向到继续流程，卡片历史详情页及其分页被移除。
- 2026-08-14 验证：`npm run check`、`npm run build`、`npm run format:check`、`git diff --check` 全部通过；全量 Vitest 为 18 个测试文件、225 项测试全部通过。

### v2.1.2 项目群创建存活性修复

- 创建项目群时明确把唯一授权用户设置为群主，并把机器人设置为群管理员，避免飞书虽然返回 `chat_id`、群却因没有稳定成员而立即解散。
- 新群在写入 SQLite 和生成 AppLink 前必须重新通过群状态检查；群主暂时缺席时自动补加成员，`dissolved`、`missing` 或持续不可用的群不会保存为项目入口。
- 首次创建和重建均使用新的随机幂等键，避免再次拿到先前已经解散的群。该修复不增加新的飞书权限要求，仍使用 2.0.4 所列的建群、读群和成员管理权限。
- 2026-08-14 验证：`npm run check`、`npm run build`、`npm run format:check`、`git diff --check` 全部通过；全量 Vitest 为 18 个测试文件、227 项测试全部通过。
