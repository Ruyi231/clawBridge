...
任务开始后，Bridge 会在原话题创建一张完整的 CardKit 轮次卡片：上方固定显示本轮用户文字、两列图片缩略预览与折叠式附件清单，下方显示 Codex 状态和回答。`item/agentMessage/delta` 以约 400 ms 节流原位更新到同一张卡片；完成时全量更新并冻结这张卡，不再另发一张结果卡。历史恢复复用同一个轮次卡片渲染器，并通过持久化的 Codex turn ID 找回该轮附件清单；仍可用的飞书图片 Key 会继续显示预览，Codex 最终回答中链接且仍然存在的项目内产物也会重新上传到对应历史轮次。流式卡片创建或更新失败不会让 Codex 任务失败，最终文本仍通过持久 outbox 可靠发送。SQLite 会保存最近的增量正文、最终正文与摘要，Bridge 重启后任务中心仍可查询。
...
Codex 也可以把产物发回飞书。Bridge 只识别最终回答中明确使用 Markdown 链接或图片语法引用、且解析后仍位于当前项目真实目录内的文件；网页 URL、目录外路径、符号链接逃逸、空文件、超限文件以及常见密钥文件会被忽略。图片（飞书限制内）直接嵌入同一张轮次卡片，其他文件作为该话题中的原生可下载附件发送，卡片中的“Codex 产物”区域会显示交付状态。可通过 `bridge.outputArtifacts.enabled` 关闭，或用 `bridge.outputArtifacts.maxFiles` 限制每轮数量。
