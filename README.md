...
当前仓库已完成 Phase 0–2 以及 ClawBridge 2.0-A/B/C 的本地实现：除原有单聊控制台、项目/对话管理和持久队列外，还增加了项目群/对话话题隔离、任务中心、CardKit 流式任务卡、卡片新建项目，以及按 Codex 对话持久保存的模型与推理强度。需要人工批准 Codex 工具调用的审批卡片仍属于后续里程碑。
