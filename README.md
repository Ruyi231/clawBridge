...
3. 进入项目群后，使用群内项目控制卡继续已有对话、新建对话、查看项目任务或设置当前对话模型。
...
选择项目后会出现“项目群”按钮。已存在的项目群使用纯 AppLink 一次跳转，不再同时触发耗时回调；选择项目时会先检查并准备群，因此退出丢弃后重新选择项目，也只需点击一次“项目群”即可进入新群。群主聊天流只用于项目控制卡，可在群内查看/继续对话、新建对话、查看项目任务和设置当前对话模型。每个 Codex 对话仍单独对应一个飞书话题：继续已有对话时会恢复历史，新建对话时先建立待绑定话题，首条任务启动后再绑定真实 Codex 线程。以后在该话题中直接发普通文本，Bridge 会按 `群 chat_id + 话题 root_id` 固定路由，入队、流式进度、完成、失败和停止结果也只回复到原话题。未登记的群、群主聊天流中的普通文本、未知话题、错误项目或其他发送者都不会执行 Codex。
