...
2. 点击“选择对话”，继续已有 Codex 对话；也可以点击“新对话”，立即创建新对话且无需手工填写名称。
...
6. 点击“全部任务”查看跨项目任务，点击“项目任务”只查看当前项目；两种任务中心都支持分页和刷新。
7. 已选择对话时，点击“模型设置”可从本机 Codex `model/list` 返回的可用模型中选择模型与推理强度。设置只作用于该 Codex 对话；任务入队时会固化设置，之后修改不会改变旧任务。

项目列表中的“新建项目”会打开飞书表单。填写项目名称并提交后，Bridge 会在 `allowedRoots` 的首个目录下创建一个受路径策略约束的新目录，自动登记、选择项目并创建项目群；需要 `allowCreateDirectory: true`。表单能力要求飞书客户端支持输入框和 `form_submit`。
...
选择项目后还会出现“项目群”按钮。首次点击会创建只包含当前用户和机器人的私有普通群；群主聊天流只用于项目控制卡，可在群内查看对话、新建对话、查看项目任务和设置当前对话模型。每个 Codex 对话仍单独对应一个飞书话题：进入或新建对话时，Bridge 会先发送话题根消息，再以 `reply_in_thread` 建立独立话题。以后在该话题中直接发普通文本，Bridge 会按 `群 chat_id + 话题 root_id` 固定路由到绑定对话，入队、流式进度、完成、失败和停止结果也只回复到原话题。未登记的群、群主聊天流中的普通文本、未知话题、错误项目或其他发送者都不会执行 Codex。

2.0.5 会在再次打开旧项目群时把旧 `thread` 消息模式迁移为 `chat` 控制台模式，但不会合并或删除已有对话话题。群内控制卡只允许操作当前群绑定的项目；“返回项目”回到当前项目控制卡，不会跳回单聊项目选择器。
