...
当前仓库已完成 Phase 0–2 以及 ClawBridge 2.0-A/B 的本地实现：除原有单聊控制台、项目/对话管理和持久队列外，还增加了“一个项目一个私有飞书群、一个 Codex 对话一个群话题”的隔离路由、全局/项目任务中心，以及由 Codex assistant delta 驱动的 CardKit 流式任务卡。需要人工批准 Codex 工具调用的审批卡片和模型选择仍属于后续里程碑。
